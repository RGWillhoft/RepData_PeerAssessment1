...
So the 5-minute interval with the maximum average steps is `r max_interval`. 
...
So we see that there are `r na_values` **NA**s in the dataset, which represents `r na_percent`% of the total `steps` values.
...
(`r sprintf("%0.0f", sum(steps_per_interval))`).
...
|        | original    | imputed     |
| ------ | ----------- | ----------- |
| mean   | `r sprintf("%0.0f",mean1)`   | `r sprintf("%0.0f",mean2)`   |
| median | `r sprintf("%0.0f",median1)` | `r sprintf("%0.0f",median2)` |
